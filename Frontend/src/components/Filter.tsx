...
const Filter: React.FC<FilterProp> = ({ onChange }): JSX.Element => {
    ...
    useEffect(() => {
        getTypes().then((data) => {
            // Ensure `data` does not include `type`
        ...
        <div className='filter card align-self-start bg-glass bg-mid-white' style={{ zIndex: 5 }}>
            <div className='card-body'>
                <div className='input-group mb-3'>
                    <input
                        type='text'
                        className='form-control border-dark bg-deep-white'
                                        ...
                                        {tags[tag as keyof FilterData].map((value: string | number, index: number) => (
                                            <span key={index} className="form-check m-1 cursor-pointer" style={{ display: 'inline-block' }}>
                                                <label className="form-check-label">
                                                    ...
                                                    <input
                                                        type="checkbox"
                                                        className='form-check-input border-2 border-dark bg-dark rounded-pill p-2 cursor-pointer'
                                                        onChange={() => {
                                                            const current = filterData[tag as keyof FilterData];
